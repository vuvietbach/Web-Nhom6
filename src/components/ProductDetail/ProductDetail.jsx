...
function ProductDetail() {
  ...
  const addToCart = (e) =>{
    ...
    axios.post(`http://localhost:8080/cart/add-cart`,data)
    .then(response => {
  ...
  const FetchSpecificItem = async() =>{
    try {
      const response = await axios.get(`http://localhost:8080/item/get-item-specific-by-origin-id/${id}`);
  ...
  try {
    const response= await axios.get(`http://localhost:8080/item/get-item-by-id/${id}`);
  ...
  const getSeller = async () => {
    try {
      ...
      const response = await axios.get(`http://localhost:8080/seller/get-seller-by-id/${_item.seller_id}`);
  ...
  const getSimilarProduct = async () => {
    try {
      const _item = JSON.parse(localStorage.getItem("item"));
      const response = await axios.get(`http://localhost:8080/item/get-item-by-category/${_item.category_id}`);
  ...
  const getComments = async () => {
    try {
      const _item = JSON.parse(localStorage.getItem("item"));
      const response = await axios.get(`http://localhost:8080/rate/get-rate/${_item.id}`);
  ...
  return (
    ...
    <div className="product-wrapper">
      <div className="image-box">
          <img src={currentItem.img} className='product-image' alt="Ảnh sản phẩm" />
      </div>
          ...
          <div className="right">
            ...
            <div className="rating-and-follower">
              <div className="rating">
                <div className='rating-title'>{convertValue(seller.rating)} / 5<GradeIcon style={{
                    color:'rgb(255,193,32)',
                    width: '16px',
                    height: '16px'
                  }}></GradeIcon>
                </div>
                <div className='subtitle'>{convertValue(seller.review_count)}+</div>
              </div>
        ...
        <div className="slideContainer" ref={slideContainerRef}>
          {similarProduct.slice(0,18).map((item,index) =>(
            <div className="product" onClick={() => {
              ...
              <img src={item.img} alt="" style={{width:"176px", height:"176px"}} />
              <div className="info">
                <span style={{color:"#ff424e"}}>{item.price} ₫</span>
                <h4 className='productName'>{item.name}</h4>
                <div className="item_review">
                  <span style={{color:"#808089"}}>{item.rate} <StarIcon style={{color:'rgb(255,213,46)'}} /></span> 
                  <div style={{margin:"0 5px 4px 5px",}}>|</div>
                  <div style={{fontSize:"12px",marginTop:"2px"}}>Đã bán {item.number_sold}</div>
                </div>
              </div>
            </div>
          ))}                          
    ...
    <div className="customer-review">
      ...
      /> */}
       <Pagination count={Math.ceil(comments.length/itemsPerPage)} page={currentPage} onChange={handleChangePage} color='primary' style={{float:"right"}}/>
