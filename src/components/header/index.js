...
export default function Header() {
    ...
    const thucPhamLink = thucPham.map((item, index) => {
        return <Link href='#' underline='none' sx={{mr:'10px', color:'text.secondary', fontSize:'0.9rem'}}>{item}</Link>
    })
    return (
        <div style={{margin: '0 auto', width: 'var(--max-content-width)'}}>
            <Box sx={{height:'40px', display:'flex', alignItems:'center', overflow:'auto'}}>
                <img src={TikiLogo} style={{height: '40px'}} alt="tiki logo"/>
                <div style={{display:'flex', alignItems:'center', flexGrow:'1'}}>
                    <div style={{width:'650px', height:'35px', margin:'0 auto', display:'flex', alignItems:'center', border:'1px solid grey', borderRadius:'7px'}}>
